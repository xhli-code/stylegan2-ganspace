...
def save_img():
    '''
    文件夹命名方式
    model(使用模型) + estimator(使用算法[pca, fbpca, cupca, spca, ica])
    + use_w + layer(使用层)

    图片命名方式
    class(类别) + ls(layer start) + le(layer end)
    + sl(slider 记录组件大于0.0的) + tr(truncation)
    + b(batchsize) + s(seed)
    '''

    slider_flag = ''
    for i in range(len(ui_state.sliders)):
        if(ui_state.sliders[i].get() != 0.0):
            slider_flag += f'sl{i}_{ui_state.sliders[i].get()}-'

    dir_name = ''
    if(args.model):
        dir_name += f'{args.model}'
    if(args.estimator):
        dir_name += f'-{args.estimator}'
    if(args.use_w):
        dir_name += '-use_w'
    if(args.layer):
        dir_name += f'-{args.layer}'
    if(args.sparsity):
        dir_name += f'-SPCA_{args.sparsity}'

    out_dir = Path(__file__).parent / 'out'/ 'img' /f'{dir_name}'
    ...
    for i,data in enumerate(batch_res):
        # Save individual
        # outname = out_dir /  f"{time.time()}.jpg"
        outname = out_dir /  f"{ui_state.outclass.get()}-ls{ui_state.edit_layer_start.get()}-le{ui_state.edit_layer_end.get()}-" \
                             f"{slider_flag}tr{ui_state.truncation.get()}-b{i}-s{ui_state.random_seed.get()}.jpg"
...
def setup_ui():
    ...
    tk.Button(toolbar, text="Reset", command=reset_sliders).pack(anchor=tk.CENTER, fill=tk.X, padx=2, pady=2)
    tk.Button(toolbar, text="Save Img", command=save_img).pack(anchor=tk.CENTER, fill=tk.X, padx=2, pady=2)
